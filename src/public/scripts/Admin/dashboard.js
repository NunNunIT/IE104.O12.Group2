...
// 	.then(res => res.json())
// 	.then(data => {
// 		let chartTop5Data = data.chartTop5Data

// 		let productName = chartTop5Data.map(item => item.product_variant_name ?? 'null')
// 		let productQuantity = chartTop5Data.map(item => item.quantity_sold)
// 		let productRevenue = chartTop5Data.map(item => item.revenue)

// 		// BAR CHART
// 		const maxQuantity = Math.max(...productQuantity)
...
// 				}
// 			] */
// 		let chartRevenueData = data.chartRevenueData

// 		let year = chartRevenueData.map(item => item.year)
// 		let month = chartRevenueData.map(item => item.month)
// 		let yearRevenue = chartRevenueData.map(item => item.revenue)
// 		let orderSuccess = chartRevenueData.map(item => item.order_success)
// 		let orderCancel = chartRevenueData.map(item => item.order_cancel)
