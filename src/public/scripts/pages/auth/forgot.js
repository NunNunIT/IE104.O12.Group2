...
const bullets = document.querySelectorAll(".forgot__bullets span");
const images = document.querySelectorAll(".forgot__image");
...
if (element.id === 'PhoneNumber' && isValidPhoneNumber(element.value.trim())) {
    showStep2();
}
};
...
const otpInputs = document.querySelectorAll('.actual-form input[type="number"]');
...
const moveToNext = (currentInput, nextInputId) => {
    const nextInput = document.getElementById(nextInputId || currentInput.getAttribute('data-next-input'));
    if (nextInput) {
        nextInput.focus();
    }
};
...
otpInputs.forEach(input => {
    ...
    input.addEventListener('input', function () {
        ...
        if (this.dataset.previousValue && this.dataset.previousValue !== truncatedValue) {
            this.value = truncatedValue;
        }
...
document.getElementById('resendLink').addEventListener('click', function (event) {
    ...
    gsap.to([statusSpan, '#resendLink'], { opacity: 0, duration: 0.5, onComplete: function () {
        ...
        setTimeout(function () {
            // Sử dụng gsap để tạo hiệu ứng fadeOut cho cả span và nút Gửi lại mã OTP
            gsap.to([statusSpan], { opacity: 0, duration: 0.5, onComplete: function () {
                // Cập nhật nội dung của span về trạng thái bình thường
                statusSpan.textContent = 'Bạn không nhận được mã OTP?';

                // Sử dụng gsap để tạo hiệu ứng fadeIn cho cả span và nút Gửi lại mã OTP
                gsap.to([statusSpan, '#resendLink'], { opacity: 1, duration: 0.5 });
            } });
        }, 5000);
    } });
});
